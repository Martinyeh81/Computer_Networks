...
#define PORT 21461 //8140821161 (21300 + 161)
...
void client(){
    ...
    printf("<HospitalC> has TCP por %d and IP address %s for Phase 1\n", ntohs(server_addr.sin_port), s);
    printf("<HospitalC> is now connected to the admission office\n");
...
void send(){
    ...
    string totalinfo = "";
    vector<string> data = read_hospital("/Users/martinyeh/Desktop/EE450/project/HospitalC.txt");
...
void receive_result(int numb_stud) {
    ...
    for (int i=0; i<numb_stud;i++){
        ...
        printf("<Student%s> has been admitted to <HospitalC>\n", finalresult.c_str());
...
int main(int argc, char *argv[])
{
    client();
    send();
    printf("End of Phase 1 for <HospitalC>\n");
    Udpserver();
    receive_result(1);
