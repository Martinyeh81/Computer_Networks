...
#define PORT 21361 //8140821161 (21200 + 161)
...
void client(){
    ...
    printf("<HospitalB> has TCP por %d and IP address %s for Phase 1\n", ntohs(server_addr.sin_port), s);
    printf("<HospitalB> is now connected to the admission office\n");
...
void send(){
    ...
    string totalinfo = "";
    vector<string> data = read_hospital("/Users/martinyeh/Desktop/EE450/project/HospitalB.txt");
...
void receive_result(int numb_stud) {
    ...
    for (int i=0; i<numb_stud;i++){
        ...
        printf("<Student%s> has been admitted to <HospitalB>\n", finalresult.c_str());
...
int main(int argc, char *argv[])
{
    client();
    send();
    printf("End of Phase 1 for <HospitalB>\n");
    Udpserver();
    receive_result(1);
