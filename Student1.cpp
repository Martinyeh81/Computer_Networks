...
#define PORT 21561 //8140821161 (21400 + 161)
...
void send(){
    ...
    string totalinfo = "";
    vector<string> data = read_hospital("/Users/martinyeh/Desktop/EE450/project/student1.txt");
    ...
    totalinfo = totalinfo + "student1";
    strcpy(buf, totalinfo.c_str());
    if ((numbytes = send(TcpSockfd, buf, MAXDATASIZE-1, 0)) > 0) {
        printf("Completed sending application for <Student1>.\n");
    }
...
void receive_result() {
    ...
    printf("<Student1> has received the application result is %s\n", finalresult.c_str());
